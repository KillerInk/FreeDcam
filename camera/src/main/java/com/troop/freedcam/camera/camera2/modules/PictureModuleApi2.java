...
import com.troop.freedcam.camera.camera2.modules.capture.ByteImageCapture;
import com.troop.freedcam.camera.camera2.modules.capture.CaptureController;
import com.troop.freedcam.camera.camera2.modules.capture.JpegCapture;
import com.troop.freedcam.camera.camera2.modules.capture.RawImageCapture;
import com.troop.freedcam.camera.camera2.modules.capture.StillImageCapture;
import com.troop.freedcam.camera.camera2.modules.helper.CaptureType;
import com.troop.freedcam.camera.camera2.modules.helper.FindOutputHelper;
...
public class PictureModuleApi2 extends AbstractModuleApi2 implements RdyToSaveImg
{
    ...
    public PictureModuleApi2(Camera2Controller cameraUiWrapper, Handler mBackgroundHandler, Handler mainHandler) {
    ...
    private void preparePreviewTextureView(int orientationToSet, Size previewSize) {
       ...
       /* if (cameraUiWrapper.getPreviewSurface() != null)
        ...
        else
        {
            ...
            cameraUiWrapper.captureSessionHandler.SetTextureViewSize(w, h, or,false);
    ...
    private void setOutputSizesAndCreateImageReader() {
        ...
        if (TextUtils.isEmpty(picFormat)) {
            picFormat = ContextApplication.getStringFromRessources(R.string.pictureformat_jpeg);
    ...
    protected void createImageCaptureListners() {
        ...
        if (captureType == CaptureType.Jpeg || captureType == CaptureType.JpegDng16 || captureType == CaptureType.JpegDng10)
            byteImageCapture = new JpegCapture(new Size(output.jpeg_width,output.jpeg_height),false,cameraUiWrapper.getFileListController(),this,".jpg",max_images);
        else
        {
            ...
            byteImageCapture = new JpegCapture(smallestImageSize,false,cameraUiWrapper.getFileListController(),this,".jpg",max_images);
        }
        ...
        if (captureType == CaptureType.Yuv) {
            ...
            ByteImageCapture yuvimgcapture = new ByteImageCapture(s, ImageFormat.YUV_420_888,false,cameraUiWrapper.getFileListController(),this,".yuv",max_images);
            captureController.add(yuvimgcapture);
        }

        if (output.raw_format != 0 && isDngCapture())
        {
            RawImageCapture rawImageCapture = new RawImageCapture(new Size(output.raw_width,output.raw_height),output.raw_format,false,cameraUiWrapper.getFileListController(),this,".dng",max_images);
        ...
        {
            ByteImageCapture byteImageCapture1 = new ByteImageCapture(new Size(output.raw_width,output.raw_height),output.raw_format,false,cameraUiWrapper.getFileListController(),this,".bayer",max_images);
    ...
    private void getCaptureType() {
        if (picFormat.equals(ContextApplication.getStringFromRessources(R.string.pictureformat_jpeg))) {
            captureType = CaptureType.Jpeg;
        }
        if (picFormat.equals(ContextApplication.getStringFromRessources(R.string.pictureformat_yuv))) {
            captureType = CaptureType.Yuv;
        }

        if (picFormat.equals(ContextApplication.getStringFromRessources(R.string.pictureformat_dng16)) || picFormat.equals(ContextApplication.getStringFromRessources(R.string.pictureformat_jpg_p_dng))) {
            Log.d(TAG, "ImageReader RAW_SENSOR");
            if (picFormat.equals(ContextApplication.getStringFromRessources(R.string.pictureformat_dng16))) {
                captureType = CaptureType.Dng16;
            } else {
                captureType = CaptureType.JpegDng16;
            }
        }
        else if (picFormat.equals(ContextApplication.getStringFromRessources(R.string.pictureformat_dng10))) {
            Log.d(TAG, "ImageReader RAW10");
            captureType = CaptureType.Dng10;
        }
        else if (picFormat.equals(ContextApplication.getStringFromRessources(R.string.pictureformat_dng12))) {
            Log.d(TAG, "ImageReader RAW12");
            captureType= CaptureType.Dng12;
        }
        else if (picFormat.equals(ContextApplication.getStringFromRessources(R.string.pictureformat_bayer))) {
            Log.d(TAG, "ImageReader BAYER16");
            captureType = CaptureType.Bayer16;
        }
        else if (picFormat.equals(ContextApplication.getStringFromRessources(R.string.pictureformat_bayer10)))
        {
    ...
    protected void captureStillPicture() {
        ...
        if (cameraUiWrapper.getParameterHandler().get(SettingKeys.LOCATION_MODE).GetStringValue().equals(ContextApplication.getStringFromRessources(com.troop.freedcam.camera.R.string.on_)))
        {
            currentLocation = cameraUiWrapper.getCurrentLocation();
        ...
        for (int i = 0; i< captureController.getImageCaptures().size();i++)
        {
            ...
            if (currentCaptureHolder instanceof StillImageCapture) {
                ...
                stillImageCapture.setOrientation(cameraUiWrapper.getDeviceOrientation());
        ...
        cameraUiWrapper.captureSessionHandler.SetCaptureParameter(CaptureRequest.JPEG_ORIENTATION, cameraUiWrapper.getDeviceOrientation());
    ...
    {
        if (BurstCounter.getBurstCount() > 1)
            return cameraUiWrapper.getFileListController().getNewFilePath(SettingsManager.getInstance().GetWriteExternal(), "_" + BurstCounter.getImageCaptured());
        else
            return cameraUiWrapper.getFileListController().getNewFilePath(SettingsManager.getInstance().GetWriteExternal(),"");
    }
