...
Func merge_temporal(Func imgs, Func alignment, Expr minoffset, Expr maxoffset, Expr l1mindist,Expr l1maxdist,Expr width, Expr height,Expr count) {
    ...
    Var ix, iy, tx, ty, n,tyi,ixi,tyii,ixii;
    ...
    offset = clamp(P(alignment(tx, ty, n)), P(minoffset, minoffset), P(maxoffset, maxoffset));
    ...
    total_weight(tx, ty) = sum(weight(tx, ty, r1)) + 1.0f;              // additional 1.f accounting for reference image
