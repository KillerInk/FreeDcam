 ...
 */
Func black_white_level(Func input, Param<int> bp, Param<int> wp) {
    ...
    output(x, y) = u16_sat(((i32(input(x, y)) - bp)/wp) * wp /* * white_factor*/);
 ...
 */
Func white_balance(Func input, Expr width, Expr height, Param<float> wb_r, Param<float> wb_g1, Param<float> wb_g2, Param<float> wb_b) {
    ...
    output(r.x * 2    , r.y * 2    ) = u16_sat(wb_r  * f32(input(r.x * 2    , r.y * 2    )));   // red
    output(r.x * 2 + 1, r.y * 2    ) = u16_sat(wb_g1 * f32(input(r.x * 2 + 1, r.y * 2    )));   // green 0
    output(r.x * 2    , r.y * 2 + 1) = u16_sat(wb_g2 * f32(input(r.x * 2    , r.y * 2 + 1)));   // green 1
    output(r.x * 2 + 1, r.y * 2 + 1) = u16_sat(wb_b  * f32(input(r.x * 2 + 1, r.y * 2 + 1)));   // blue
 ...
 */
Func interleaved_u8bit(Func input) {

    Func output("interleaved_8bit_output");
 ...
 */
Func finish(ImageParam input, Param<int> bp, Param<int> wp, Param<float> wb_r, Param<float> wb_g1, Param<float> wb_g2, Param<float> wb_b, Param<float> compression, Param<float> gain) {

	int denoise_passes = 1;
	float contrast_strength = 5.f;
	int black_level = 64;
	float sharpen_strength = 2.f;
	Expr width = input.dim(0).max();
	Expr height = input.dim(1).max();
	...
	Func white_balance_output = white_balance(black_white_level_output, width, height, wb_r, wb_g1,wb_g2,wb_b);
	...
	Func tone_map_output = tone_map(srgb_output, width, height, compression, gain);
	...
	Var c, x, y;
	return interleaved_u8bit(contrast_output);
}
