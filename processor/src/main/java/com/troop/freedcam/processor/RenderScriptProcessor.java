...
package com.troop.freedcam.processor;
...
import com.troop.freedcam.eventbus.updater.HistogramUpdater;
...
public class RenderScriptProcessor implements RenderScriptProcessorInterface
{
    ...
    public RenderScriptProcessor(RenderScriptManager renderScriptManager, int imageformat)
    {
        ...
        HistogramUpdater.updateHistogramVisibility(View.GONE);
    ...
    public void setHistogramEnable(boolean enable) {
        ...
        if (processHistogram)
            HistogramUpdater.updateHistogramVisibility(View.VISIBLE);
        else
            HistogramUpdater.updateHistogramVisibility(View.GONE);
    }
            ...
            synchronized (workLock) {
                ...
                if (peak && processHistogram && processClipping)
                {
                    ...
                    if (framescount % HISTOGRAM_UPDATE_RATE == 0) {
                        ...
                        HistogramUpdater.sendHistogram(histodataR);
                        framescount = 0;
                    }
