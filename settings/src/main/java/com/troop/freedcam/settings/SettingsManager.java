...
public class SettingsManager implements SettingsManagerInterface {
    ...
    public void init()
    {
        ...
        if (isInit)
            return;
        isInit = true;
        File ext = ContextApplication.getContext().getExternalFilesDir(null);
        if (ext != null && ext.exists())
            settingsStorage = new SettingsStorage(ext);
        else
            settingsStorage = new SettingsStorage(ContextApplication.getContext().getFilesDir());
    ...
    private void parseXml() {
        ...
        //first time init
        matrixes = parser.getMatrixes(ContextApplication.getContext().getResources(),settingsStorage.appdataFolder);
        ...
        if (settingsStorage.getDevice() == null || TextUtils.isEmpty(settingsStorage.getDevice()))
        {
            ...
            parser.parseAndFindSupportedDevice(ContextApplication.getContext().getResources(),matrixes,settingsStorage.appdataFolder);
    ...
    public boolean appVersionHasChanged(int version)
    {
    ...
    public String GetCurrentModule()
    {
        if (TextUtils.isEmpty(getApi(SettingKeys.Module).get()))
            return ContextApplication.getStringFromRessources(R.string.module_picture);
