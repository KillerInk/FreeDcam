...
package com.troop.freedcam.gallery.helper;
...
public class BitmapHelper
{
    ...
    private Bitmap createCacheImage(BaseHolder file, boolean thumb)
    {
        ...
        if (response == null && file.getName() !=null)
        {
                    ...
                    )
            {
                ...
                response = getBitmap(context,file,options);
            }
            else if (file.getName().toLowerCase().endsWith(FileEnding.MP4)) {
                try {
                    response = getVideoThumb(context,file);
                } catch (IOException e) {
                    e.printStackTrace();
                }
                //
            ...
            {
                try {
                    response = getBitmapFromDng(context,file);
                }
    ...
    public static Bitmap getBitmap(Context context, BaseHolder baseHolder,Options options)
    {
        if (baseHolder instanceof FileHolder)
        {
            ...
            return ImageLoader.getBitmap(context,options,fh.getFile());
        }
        if (baseHolder instanceof UriHolder)
        {
            UriHolder uh = (UriHolder) baseHolder;
            return ImageLoader.getBitmap(context,options,uh.getMediaStoreUri());
        }
        return null;
    }
    ...
    public static Bitmap getVideoThumb(Context context, BaseHolder baseHolder) throws IOException {
        ...
        {
            FileHolder fh = (FileHolder) baseHolder;
            return ImageLoader.getVideoThumb(context,fh.getFile());
        }
        if (baseHolder instanceof UriHolder)
        {
            UriHolder uh = (UriHolder) baseHolder;
            return ImageLoader.getVideoThumb(context,uh.getMediaStoreUri(),uh.getID());
        }
        return null;
    }
    ...
    public static Bitmap getBitmapFromDng(Context context, BaseHolder baseHolder) throws IOException {
        ...
        {
            FileHolder fh = (FileHolder) baseHolder;
            return ImageLoader.getBitmapFromDng(context,fh.getFile());
        }
        if (baseHolder instanceof UriHolder)
        {
            UriHolder uh = (UriHolder) baseHolder;
            return ImageLoader.getBitmapFromDng(context,uh.getMediaStoreUri());
        }
        return null;
    }
