...
public class FocusManualParameter extends  BaseManualParameter
{
    ...
    private static String TAG =FocusManualParameter.class.getSimpleName();
    ...
    public FocusManualParameter(HashMap<String, String> parameters, String value, String maxValue, String MinValue, I_CameraHolder cameraHolder, AbstractParameterHandler camParametersHandler) {
        ...
        if (DeviceUtils.IS_DEVICE_ONEOF(DeviceUtils.ZTE_DEVICES)  || DeviceUtils.IS(DeviceUtils.Devices.Xiaomi_RedmiNote)|| DeviceUtils.IS_DEVICE_ONEOF(DeviceUtils.MI3_4) || DeviceUtils.IS(Devices.LenovoK920) )
        {
        ...
        else if(DeviceUtils.IS(Devices.LenovoK920) || DeviceUtils.IS_DEVICE_ONEOF(DeviceUtils.QC_Manual_New))
        {
            this.isSupported = true;
    ...
    protected void setvalue(final int valueToSet)
    {
        ...
        if (DeviceUtils.IS_DEVICE_ONEOF(new DeviceUtils.Devices[]
                {
                        DeviceUtils.Devices.ZTE_ADV, Devices.ZTEADVIMX214, Devices.ZTEADV234, Devices.XiaomiMI3W, Devices.XiaomiMI4W, Devices.Xiaomi_RedmiNote, Devices.LenovoK920,Devices.SonyM4_QC,Devices.Aquaris_E5
                }))
        {
            if(valueToSet != 0)
            {
