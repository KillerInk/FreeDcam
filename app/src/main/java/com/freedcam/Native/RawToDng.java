...
/*    private Byte[] raw2byte()
    ...
    public void WriteDNG(DeviceUtils.Devices device, AppSettingsManager appSettingsManager)
    {
        if (device != null)
        {
            DngSupportedDevices.DngProfile profile = new DngSupportedDevices().getProfile(device, (int)GetRawSize(),appSettingsManager);
