...
char* copyString(JNIEnv* env, jstring input)
{
...
float* copyfloatArray(JNIEnv *env, jfloatArray input)
{
...
int* copyintArray(JNIEnv *env, jintArray input)
{
    int size = env->GetArrayLength((jarray)input);
...
unsigned char* copyByteArray(JNIEnv* env, jbyteArray input)
{
    unsigned char* out = new unsigned char[env->GetArrayLength((jarray)input)];
    jbyte * bytes = env->GetByteArrayElements(input,NULL);
    memcpy(out,bytes,writer.rawSize);
