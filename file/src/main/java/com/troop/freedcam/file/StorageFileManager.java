...
public class StorageFileManager
{
    ...
    public StorageFileManager(Context context)
    {
    ...
    public ArrayList<BaseHolder> getDCIMDirs()
    {
        ...
        File intdcim = new File(fileApiStorageDetector.getInternalSD(),context.getString(R.string.DCIMFolder));
        readSubFolders(subDcimFolders,intdcim,false);
        if (fileApiStorageDetector.getExternalSD() != null)
        {
            File extDcim =  new File(fileApiStorageDetector.getExternalSD(),context.getString(R.string.DCIMFolder));
    ...
    public  ArrayList<BaseHolder> getFreeDcamDCIMDirsFiles()
    {
        ...
        File intDcimFreed = new File(fileApiStorageDetector.getInternalSD(), context.getString(R.string.FreeDcamFolder));
        if (intDcimFreed != null && intDcimFreed.exists())
            readFilesFromFolder(intDcimFreed, f, FileListController.FormatTypes.all, false);
        if (fileApiStorageDetector.getExternalSD() != null)
        {
            File extDcimFreed = new File(fileApiStorageDetector.getExternalSD(), context.getString(R.string.FreeDcamFolder));
        ...
        if (subfolders != null)
        {
            for (File f : subfolders)
            {
                if (!f.isHidden() && f.isDirectory())
                    listToAdd.add(new FileHolder(context,f, external));
            }
        }
    }
        ...
        for (File f : folderfiles) {
            if (!f.isHidden()) {
                if (formatsToShow == FileListController.FormatTypes.all && (
                    ...
                    list.add(new FileHolder(context,f,external));
                else if (formatsToShow == FileListController.FormatTypes.dng && f.getName().toLowerCase().endsWith(FileEnding.DNG))
                    list.add(new FileHolder(context,f,external));
                else if (formatsToShow == FileListController.FormatTypes.raw && f.getName().toLowerCase().endsWith(FileEnding.RAW))
                    list.add(new FileHolder(context,f,external));
                else if (formatsToShow == FileListController.FormatTypes.raw && f.getName().toLowerCase().endsWith(FileEnding.BAYER))
                    list.add(new FileHolder(context,f,external));
                else if (formatsToShow == FileListController.FormatTypes.jps && f.getName().toLowerCase().endsWith(FileEnding.JPS))
                    list.add(new FileHolder(context,f,external));
                else if (formatsToShow == FileListController.FormatTypes.jpg && f.getName().toLowerCase().endsWith(FileEnding.JPG))
                    list.add(new FileHolder(context,f,external));
                else if (formatsToShow == FileListController.FormatTypes.mp4 && f.getName().toLowerCase().endsWith(FileEnding.MP4))
                    list.add(new FileHolder(context,f,external));
            }
        }
        sortFileHolder(list);
